...
KNOWLEDGE_ITEMS = [
    ...
    },
    {
        ...
        "content": "Jurgen Appelo's unFIX organisational pattern set distinguishes four value-stream types (Customer Stream, Internal Stream, Innovation Stream, Operational Stream). Core principle: organise around end-to-end flows of value to a stakeholder, not around departments or functions. Each value stream is owned by a Crew (cross-functional team) that crosses departmental boundaries and continuously improves the stream (lean roots). For AI Client Blueprints this becomes a powerful organising principle: AI is designed *into* a value stream, not bolted onto a department. This naturally aligns with FDE-principles (embed in a stream, not in a department) and with Obeidat's value-stream-first discovery playbook.",
        "summary_nl": "Jurgen Appelo's unFIX organisatie-patroon onderscheidt vier value-stream-types (Customer Stream, Internal Stream, Innovation Stream, Operational Stream). Kernprincipe: organiseer rond end-to-end flows van waarde naar een stakeholder, niet rond afdelingen of functies. Elke value stream is in eigendom van een Crew (cross-functioneel team) die door silo's heen snijdt en de stream continu verbetert (lean-oorsprong). Voor AI Client Blueprints wordt dit een krachtig organiserend principe: AI wordt *in* een value stream ontworpen, niet aan een afdeling vastgeplakt. Sluit natuurlijk aan op FDE-principes (embed in een stream, niet in een afdeling) en op Obeidat's value-stream-first discovery-playbook.",
        "curation_score": 8,
