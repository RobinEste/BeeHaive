...
checklist:
  ...
  - "Bevat de audit-logging per tool-aanroep gebruiker, parameters, autorisatiebeslissing en resultaat, en wordt die minimaal zes maanden bewaard?"
...
evidence:
  ...
  - "Audit-log van elke tool-aanroep met gebruiker, parameters, autorisatiebeslissing en uitkomst, minimaal zes maanden bewaard (EU AI Act Art. 12)"
...
Het model voert nooit zelf code uit. Het retourneert een gestructureerd tool-call-blok; de toepassing is altijd in controle van de werkelijke uitvoering. Dat is een kritische veiligheidsgrens: een agent die niet zelf kan uitvoeren, kan ook niet buiten zijn grenzen treden.

### De stroom in vijf stappen

De aanroep van een tool door een [agent](/begrippen#agent) verloopt in vijf stappen die zich kunnen herhalen tot de taak klaar is:
...
Het JSON-schema is het enige interface waarmee een model weet wat een tool doet, wanneer hij die moet gebruiken, en met welke parameters. Een slecht schema produceert systematische fouten; een goed schema is de basis van betrouwbare tool-keuze. Schrijf het schema dus alsof je een API-contract met een externe partij sluit: expliciet, getest, met versiebeheer.

### Wat in elk goed schema staat

- **Naam**: in snake_case, actie-georiënteerd (`get_customer_orders`, niet `data` of `helper`). Beschrijf de actie, niet de implementatie. Maximaal 64 tekens.
...
- **Parameters**: gebruik `enum` voor eindige waardelijsten (dat elimineert hallucinaties waar het model waardes verzint), gebruik `integer` waar gehele getallen vereist zijn (niet `number`), markeer `required` expliciet, voeg constraints toe in het schema zelf (`maxLength`, `minimum`, `maximum`, `format`), en gebruik `additionalProperties: false` om onverwachte velden te blokkeren.
- **Strikte modus**: Anthropic biedt een vlag waarmee Claude garandeert dat zijn tool-aanroepen exact het schema matchen. Dat lost één klasse fouten op (verkeerd geformatteerde JSON), maar niet alle.

### Wat strict mode niet oplost

Strict mode garandeert de *vorm* van de output: geldige JSON, correcte veldnamen, juiste types. Het garandeert niet de *inhoud*. Een model kan syntactisch perfecte JSON produceren met semantisch onjuiste of gehallucineerde waardes; bijvoorbeeld een correct-getypeerde `customer_id` die niet bestaat, of een prijs die plausibel oogt maar uit lucht is gegrepen.
...
- **Diep nesten**: meer dan vijf nesting-niveaus maakt het model meetbaar minder betrouwbaar. Houd schema's plat.
...
Naarmate een tool-catalogus groeit (bij meer dan tien tools is het vrijwel altijd zo) wordt versiebeheer kritisch. Behandel tool-schema's als code: in Git, met pull requests, semantic versioning. Een major-versie bij breaking changes (parameternaam wijzigt), een minor-versie bij nieuwe optionele parameters, een patch bij verbeteringen aan de beschrijving. Zonder deze discipline ontstaat scope-creep waarin niemand meer weet welk veld op welke versie verplicht was.
...
- **Governance**: rechten kunnen per namespace worden ingericht (deze agent mag wel `support.*`, geen `finance.*`), in plaats van per losse tool.
- **Observability**: dashboards en alarmen werken op namespace-niveau, niet op honderden losse tools.
...
- **Zoeken en RAG**: web-zoekopdrachten, vector-database-queries, hybride zoeken (keyword + vector + reranker). Voor zero-ops een managed vector-database (Pinecone); pgvector wanneer PostgreSQL al in de stack zit.
- **Code uitvoeren**: hoog-risico, altijd in een afgeschermde omgeving. Anthropic's server-side code-uitvoering draait op Anthropic-infrastructuur; zelf-gehoste sandboxen (Docker) geven meer controle maar meer operationele last.
- **Databases en interne systemen**: queries via een ORM of geparametriseerde queries, nooit ruwe SQL met gebruikers-input. Schrijf-operaties krijgen expliciete transacties; voor leeszware queries een aparte tool-gebruiker met alleen-lezen-rechten.
- **Browser en computerbediening**: van gerichte browser-automatisering (Playwright) tot volledige bureaubladbediening via Anthropic's Computer Use. De catch-all voor systemen zonder API. Trage roundtrips (elke actie vraagt een schermafbeelding) en complex te beveiligen, dus alleen waar geen smallere tool bestaat.
- **Externe APIs en RPA**: REST-aanroepen, webhooks, betalingen, communicatieplatformen. Let op autorisatie-scopes (alleen wat nodig is), idempotentie bij retries, en de snelheidslimieten van de externe dienst.

</BBDisclosure>

<BBDisclosure title="Tools falen altijd op het verkeerde moment">

Netwerken hebben hapering. APIs geven snelheidslimiet-fouten. Modellen verzinnen tool-parameters. Productiestabiliteit is niet een functie van "minder fouten verwachten", maar van bewuste foutafhandeling op elke laag. AI Workflow Lab vatte het bondig samen:
...
### Nieuwe poging met exponentieel oplopende wachttijd plus jitter

Drie nieuwe pogingen voor tijdelijke fouten (snelheidslimiet, timeout, server-fouten in de 500-reeks). Maximaal één tot twee pogingen bij kwaliteitsfouten; meer pogingen leveren dezelfde hallucinatie nog een paar keer. Geen retry bij permanente fouten (autorisatie-fouten, slechte input, context-window-overflow); die fouten lossen zich niet op door wachten.

De wachttijd loopt exponentieel op (kort bij de eerste poging, langer bij de tweede, nog langer bij de derde) en bevat jitter: een klein willekeurig getal dat voorkomt dat duizend agents tegelijk een halve seconde later opnieuw proberen en zo de net herstelde dienst opnieuw plat leggen. Een gangbaar productie-patroon in de Python Tenacity-bibliotheek (in de meeste LangChain- en LangGraph-stacks de standaard): een willekeurig-exponentiële wachttijd met start 0,25 seconde en bovengrens 6 seconden, en maximaal vier pogingen voordat naar terugval wordt overgeschakeld.
...
Een productie-casus uit een AI-agent-team rapporteerde dat consistent ingerichte foutafhandeling de betrouwbaarheid optilde van 87% naar 99,2%. Die specifieke cijfers zijn zelfrapportage en moeten niet als universele waarheid worden behandeld; de richting is consistent met algemene resilience-literatuur en past bij wat productie-systemen halen die deze patronen daadwerkelijk implementeren.
...
Omdat de hoofdtaak van de agent niet wordt verstoord, is deze aanval nagenoeg ondetecteerbaar voor reguliere monitoring. Een real-world variant met vergelijkbaar mechanisme is **CVE-2025-32711 (EchoLeak)** in Microsoft Copilot, waarbij data-exfiltratie via tool-context plaatsvond.
...
Een tweede klasse risico raakt elke organisatie die externe MCP-servers gebruikt: een vertrouwde MCP-server kan via een update onopgemerkt worden vervangen door een schadelijke versie. Deze "RUG Pull"-aanval is de software-toeleveringsketen-variant die we al kennen van npm en PyPI, alleen draait die nu op tool-niveau. Mitigatie: versies vastpinnen, schema-hashes valideren bij elke update, en MCP-servers van externe leveranciers behandelen als externe afhankelijkheden met dezelfde rigoureuze review als open-source code-pakketten.
...
Audit-logging voor AI met tools gaat verder dan klassieke toegangslogboeken. Per model-interactie horen zes velden minimaal vastgelegd te worden: geverifieerde gebruikersidentiteit en sessie-ID; een samenvatting van de interactie; elke tool-aanroep met volledige parameters; elk opgehaald document met ID en gevoeligheidsklassering; autorisatiebeslissingen inclusief weigeringen; en een classificatie van de output (voldoet die aan verwachte patronen, of wijkt hij af).

Voor hoogrisico AI-systemen in Europa is dit niet langer goede gewoonte, maar wettelijk vereist. De EU AI Act (Reg. 2024/1689) treedt op 2 augustus 2026 in werking voor hoogrisico-systemen, met boetes tot 35 miljoen euro of 7% wereldwijde jaaromzet. Twee artikelen raken Tool Integration direct.

### Artikel 12: automatische logging en traceerbaarheid

De wet eist automatische logging die volledige traceerbaarheid mogelijk maakt. Logs moeten manipulatie-bestendig zijn (een hash-keten of write-once-opslag is gangbare invulling) en minimaal zes maanden bewaard blijven. Voor agent-systemen betekent dat: elke beslissing gelogd inclusief de tool-aanroepen die de beslissing onderbouwden en de redenering eromheen, op een manier die later reconstrueerbaar is. Niet "we hebben de eindstand", maar "we kunnen aantonen hoe het systeem tot deze eindstand kwam".

### Artikel 14: menselijk toezicht en stop-mechanismen

Hoogrisico-systemen moeten menselijk toezicht mogelijk maken, inclusief de capaciteit om in te grijpen. Voor een [agent](/begrippen#agent) is dat geen abstract beginsel: het systeem moet een stop-knop of override-mechanisme hebben, en stopcondities moeten expliciet zijn gedefinieerd. Een agent zonder kill-switch en zonder duidelijke stopcondities is in een hoogrisico-context vermoedelijk niet conform.

### Audit-logging als detectie, niet alleen forensiek

De minst gebruikte toepassing van audit-logging is ook de waardevolste: een actief detectiesysteem in plaats van een forensisch instrument. Anomaliedetectie op afwijkende tool-parameters, plotselinge pieken in retrieval-volume, en triggerrates van output-filters laten zien wanneer een aanval gaande is, niet pas wanneer hij voorbij is. Een Log-To-Leak-aanval, bijvoorbeeld, blijft onder de radar van klassieke metingen (de hoofdtaak slaagt immers gewoon), maar is zichtbaar als anomalie in welke tools wanneer worden aangeroepen ten opzichte van het normale patroon van die agent.

### Nederlandse implementatie: meerdere toezichthouders, geen één-loket

Nederland heeft per ontwerpwet (april 2026) gekozen voor decentraal toezicht. Bestaande sectorale toezichthouders krijgen elk handhavingsbevoegdheid binnen hun domein: de Autoriteit Persoonsgegevens, de Autoriteit Financiële Markten, de Autoriteit Consument en Markt en sectorale waakhonden voor zorg en onderwijs. Voor agent-systemen in de zorg, financiën of overheid betekent dit dat tool-integratie-keuzes ook moeten standhouden tegenover sectorale audits, niet alleen tegenover Europese.
...
Tot eind 2024 was elke tool-integratie maatwerk: per leverancier een eigen wrapper, per framework een eigen specificatie, per upgrade een eigen migratie. In november 2024 publiceerde Anthropic het [Model Context Protocol (MCP)](/begrippen#mcp): een open standaard om AI-systemen op een uniforme manier met externe tools en databronnen te verbinden. In 2025 sloten OpenAI, Google en Microsoft zich aan; per medio 2025 zijn er meer dan 5.000 publieke MCP-servers beschikbaar (kennisbanken, agenda's, CRM's, betaalsystemen, monitoring, en zo verder).
...
- **Standaard authenticatie**: OAuth 2.1 voor remote MCP-servers (beschikbaar sinds maart 2025) maakt het mogelijk om MCP-tools veilig te koppelen aan zakelijke systemen zonder eigen autorisatie-wiel.

Gebruik standaard-invoer/uitvoer (stdio) voor ontwikkeling (eenvoudig, debugbaar) en streamable HTTP voor productie (toegang op afstand, meerdere clients tegelijk, OAuth 2.1).

### Agents als MCP-server

Een geavanceerd patroon dat in 2026 traction krijgt: agents kunnen zichzelf als MCP-server aanbieden. Andere agents roepen ze aan over hetzelfde protocol. Dat creëert een vendor-neutraal multi-agent ecosysteem zonder dat elke agent-leverancier een eigen onderling protocol verzint.
...
De aantrekkelijkste valkuil bij tool-integratie is breedte voor diepte ruilen. Vijf integraties met basisfoutafhandeling produceren vijf bronnen van productie-incidenten; één integratie met volledige foutafhandeling produceert een blauwdruk waarmee de tweede en derde sneller en veiliger kunnen volgen. Implementeer voor de eerste tool nieuwe pogingen met exponentieel oplopende wachttijd plus jitter, [circuit breaker](/begrippen#circuit-breaker) per leverancier, expliciete timeout, idempotentie bij schrijfacties, audit-logging en een meetbaar reactietijd-budget. Dit kost één tot twee weken per tool, en bespaart maanden aan productie-pijn later.
...
- **Semantisch cachen**: bij voldoende overeenkomst van een nieuwe vraag met eerder werk, een gecached antwoord teruggeven. Gerapporteerde besparingen lopen tussen 30% en 70%, sterk afhankelijk van de werklast.
...
Deze cijfers zijn richtingen, geen beloftes. De richting is wel consistent: tool-overhead is een operationele variabele, geen vaste kost.
