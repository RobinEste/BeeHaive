...
Een **waarde-stroom** (*value stream*) is de end-to-end-journey die een trigger transformeert in een uitkomst voor een stakeholder. Niet een afdeling, niet een tool, maar een volledige flow van waarde. Voorbeelden die elke organisatie kent:
...
### Crews en value-stream-types (unFIX, Jurgen Appelo)

Jurgen Appelo's unFIX-model maakt deze invalshoek expliciet. Een Crew is een cross-functioneel team dat één waarde-stroom *bezit* en continu verbetert (lean-oorsprong). unFIX onderscheidt vier waarde-stroom-types:

- **Customer Stream**: levert waarde direct aan een externe klant
- **Internal Stream**: levert waarde aan een interne afnemer (denk aan finance, HR, IT-platform)
- **Innovation Stream**: ontwikkelt nieuwe waarde-proposities
- **Operational Stream**: houdt het bestaande operationeel

Voor een Client Blueprint betekent dit: identificeer welk type stream je wilt versterken; dat bepaalt mede welke risico's, KPI's en stakeholders relevant zijn. Een Customer Stream raakt extern reputatierisico; een Internal Stream raakt vooral efficiëntie en datakwaliteit.
