...
auditExample:
  ...
  description: "We hanteren een drie-tier-aanpak. Voor interviews met klantmedewerkers gebruiken we Llama 3.1 8B lokaal via Ollama: geen data verlaat de EU, transcripten blijven op infrastructuur die wij beheren. Voor rapportgeneratie met geanonimiseerde data schakelen we over naar Claude Sonnet voor sterkere analyse, met Mistral Large 3 als juridisch beoordeelde fallback. Voor classificatie en transcript-clean-up draait Claude Haiku als kostenefficiënte werker. De keuze per tier is per use case onderbouwd op zeven dimensies en getoetst op een eigen evaluatie-set; ze is geen technische voorkeur, maar een ontwerp-beslissing met expliciete trade-offs."
evidence:
...
Wereld-modellen (*world models*) modelleren hoe de wereld reageert op acties. Ze zijn vooral relevant voor organisaties in robotica, autonome voertuigen of industriële simulatie. Vier spelers in 2025–2026:
...
De frontier-markt is in 2026 ingrijpend veranderd. Drie aanbieders convergeren op de moeilijkste benchmarks tot binnen tienden van procentpunten, terwijl hun prijzen factor 2,5 uiteenlopen: Gemini 3.1 Pro op $2 input tegenover Opus 4.7 en GPT-5.5 op $5. Wie tier-onbewust kiest, betaalt voor vergelijkbare output al snel twee tot drie keer te veel. Open-weight challengers vergroten die spreiding: DeepSeek V4 Pro ($1,74 / $3,48) presteert op SWE-Bench en GPQA Diamond binnen enkele procentpunten van Opus 4.7 en GPT-5.5, tegen drie tot zeven keer lagere kosten per miljoen tokens. De vraag *"welk model is het beste?"* heeft geen algemeen antwoord; alleen een taak-specifiek antwoord.
...
<BBDisclosure title="Kostenoptimalisatie: zes hefbomen, in volgorde">

LLM-kosten lopen op een schaal die in 2024 nog niet zichtbaar was. Een chatbot met honderdduizend dagelijkse aanroepen op GPT-5.5 Pro kan tienduizenden euro's per maand kosten; dezelfde chatbot op een tier-strategie met caching kost vaak een fractie. Zes hefbomen, in volgorde van impact:
