...
code: BB_06
name: Model Engines
introTitle: "Wat zijn Model Engines?"
...
quickStart:
  ...
  - |
    ...
    Bouw een model-agnostische gateway (bijvoorbeeld via LiteLLM of een eigen abstractielaag) zodat van model wisselen een kwestie van uren is. Activeer prompt caching waar mogelijk. Configureer monitoring op vijf pijlers: latency, kosten, kwaliteit, betrouwbaarheid en afwijkingen.
auditExample:
...
Model engines zijn de denk- en rekenkernen van een AI-oplossing: het model (bijv. Claude Opus 4.7 of Gemini 3.1) dat input verwerkt tot uitvoer, plus de operationele laag waarmee dat model wordt aangestuurd. Het gaat niet alleen om grote taalmodellen ([LLMs](/begrippen#llm)); ook [redeneer-modellen](/begrippen#redeneer-model), [multimodale modellen](/begrippen#multimodaal-model), [wereld-modellen](/begrippen#wereld-model) (*world models*), [diffusiemodellen](/begrippen#diffusiemodel), [embedding-modellen](/begrippen#embedding-model) en [klassieke machine-learning-algoritmen](/begrippen#klassieke-ml) zijn legitieme architectuurcomponenten.
...
<BBDisclosure title="Het juiste gereedschap voor de taak: zeven beslisfactoren">

De meest-gemaakte ontwerpfout in 2026 is de reflex om overal het grootste, nieuwste taalmodel van een aanbieder in te zetten (in jargon: een [*frontier-model*](/begrippen#frontier-model), zoals Claude Opus 4.7, GPT-5.5 Pro of Gemini 3.1 Pro). Dat is duurder, trager en moeilijker uit te leggen wáárom het tot een bepaald antwoord komt. Voor de meerderheid van zakelijke taken is het simpelweg het verkeerde gereedschap.

### Drie redenen waarom "het grootste model overal" niet werkt

- **Kosten**: een frontier-LLM kost $0,01–$0,50 per aanroep; [klassieke machine learning](/begrippen#klassieke-ml) $0,001–$0,01; een regelgebaseerd systeem nagenoeg nul. Bij honderdduizend aanroepen per maand scheelt dit tussen €900 en €45.000 per maand voor exact dezelfde taak.
...
- **Verklaarbaarheid**: classifiers (modellen die input in vaste categorieën indelen, zoals "spam" of "geen spam") en regressiemodellen zijn statistisch te valideren; [LLMs](/begrippen#llm) zijn juridisch nauwelijks auditeerbaar. In gereguleerde sectoren (financiën, zorg, overheid) is dat geen detail maar een blokkade.
...
- Is er onvoldoende gelabelde data voor een klassiek model? LLM met [few-shot](/begrippen#few-shot)-voorbeelden.

### Aanbevolen progressie

Als een LLM het juiste gereedschap is, bouw dan in deze volgorde op: begin met [prompt](/begrippen#prompt) engineering op een generalistisch model; voeg [RAG](/begrippen#rag) toe wanneer kenniscontext ontbreekt; pas fine-tunen toe als laatste stap, en alleen als je 1.000 of meer kwaliteitsvoorbeelden hebt én een meetbaar evaluatiecriterium. Fine-tunen zonder evaluatiepijplijn is "schieten in het donker": je weet niet of het is verbeterd, voordat het in productie staat.

</BBDisclosure>

<BBDisclosure title="Het modeltype-landschap: meer dan LLMs alleen">

Het modeltype-landschap is in 2026 breder dan ooit. Een model engine kan een [LLM](/begrippen#llm) zijn, maar even goed een [redeneer-model](/begrippen#redeneer-model), een [multimodaal model](/begrippen#multimodaal-model), een [wereld-model](/begrippen#wereld-model), een [diffusiemodel](/begrippen#diffusiemodel), een [embedding-model](/begrippen#embedding-model) of een [klassiek ML-algoritme](/begrippen#klassieke-ml). De architectuur-vraag *"welk modeltype past hier?"* gaat aan de model-keuze vooraf.
...
Een embedding-model zet tekst om naar getalreeksen waarmee semantische gelijkenis meetbaar wordt. Onmisbaar voor [RAG](/begrippen#rag), semantisch zoeken, clustering en anomaliedetectie. Kosten zijn significant lager dan generatieve LLMs: typisch $0,0001–$0,002 per miljoen [tokens](/begrippen#token). Wie RAG implementeert, gebruikt altijd een embedding-model.
...
<BBDisclosure title="Het top AI-modellen landschap april 2026: geen universele winnaar">

De [frontier](/begrippen#frontier-model)-markt is in 2026 ingrijpend veranderd. Drie aanbieders staan op de moeilijkste benchmarks tot binnen tienden van procentpunten van elkaar, terwijl hun prijzen factor 2,5 uiteenlopen: Gemini 3.1 Pro op $2 input tegenover Opus 4.7 en GPT-5.5 op $5. Wie [tier](/begrippen#tier)-onbewust kiest, betaalt voor vergelijkbare output al snel twee tot drie keer te veel. [Open-weight](/begrippen#open-weight-model) challengers vergroten die spreiding: een groep Chinese modellen — **DeepSeek V4 Pro**, **Kimi K2.6** (Moonshot AI), **GLM-5.1** (Z.ai) en **MiniMax M2.7** — presteert op coding-benchmarks binnen enkele procentpunten van Opus 4.7 en GPT-5.5, tegen vijf tot vijfentwintig keer lagere kosten per miljoen tokens. Op kennis-zware reasoning loopt de gesloten frontier nog enkele punten voor. De vraag *"welk model is het beste?"* heeft geen algemeen antwoord; alleen een taak-specifiek antwoord.

### De drie grote, drie verschillende inzetten

- **Anthropic Claude Opus 4.7**: $5 input / $25 output per miljoen [tokens](/begrippen#token), [context-window](/begrippen#context-window) van 1 miljoen tokens. Sterkste op productie-code (SWE-bench Pro 64,3%), tool-zware agents (MCP-Atlas 77,3%) en desktop-automatisering (OSWorld 78,0%).
...
**Tokenizer-inflatie.** Claude Opus 4.7 levert een nieuwe [tokenizer](/begrippen#tokenizer) die 1,0 tot 1,35 keer meer tokens gebruikt dan Opus 4.6, afhankelijk van de inhoud. De stickerprijs is gelijk gebleven, maar de effectieve kostenstijging is 10–35%. Wie alleen op stickerprijzen vergelijkt, mist deze stille kostenpost.
...
<BBDisclosure title="Wanneer kun je beter een combinatie van modellen gebruiken?">
...
- **Direct routing** (intent-based): een classificator herkent het type aanvraag binnen 50 ms en stuurt naar de juiste [tier](/begrippen#tier). Werkt goed bij duidelijk onderscheid tussen categorieën. Vereist een nauwkeurige classifier en monitoring op zijn [drift](/begrippen#drift). Tooling: LiteLLM (open-source), OpenRouter, Martian, Not Diamond, Amazon Bedrock Intelligent Prompt Routing.
...
- **Anthropic** propageert *Safety as Infrastructure*: hiërarchische orchestrator-worker-architecturen waarin een planner de taak opbreekt, een werker uitvoert en een onafhankelijke evaluator de output beoordeelt. Hun multi-[agent](/begrippen#agent) research-systeem presteert 90,2% beter dan een enkel sterk model op complexe retrievetaken.
...
Lokale modellen wisselen geen data uit met externe AI-aanbieders, maar kennen grenzen. De kwaliteit ligt over het algemeen onder die van de gesloten [frontier](/begrippen#frontier-model), al lopen de Chinese open-weight modellen (DeepSeek V4 Pro, Kimi K2.6, GLM-5.1, MiniMax M2.7) op coding-benchmarks niet ver meer achter. De ondersteuning voor agentic tooling ([MCP](/begrippen#mcp), computer-use) is beperkter dan bij de drie grote frontier-aanbieders. Lokaal draaien vraagt een eenmalige hardware-investering van €15.000 (een Mac Mini-cluster) tot €50.000 of meer (een GPU-server), maar daarna betaal je geen maandelijkse API-kosten meer; stroom- en onderhoudskosten zijn marginaal vergeleken met cloud-API-tarieven.
...
De EU AI Act trad op 1 augustus 2024 in werking, maar werkt gefaseerd: GPAI-verplichtingen sinds augustus 2025, en **brede handhaving voor hoog-risico-systemen vanaf 2 augustus 2026**, waarop het EU AI Office volledig mandaat krijgt. Boetes voor non-compliance lopen op tot **€35 miljoen of 7% van de wereldwijde jaaromzet**. Voor Nederlandse organisaties die "black-box"-modellen integreren — waarbij de leverancier weigert inzicht te geven in trainingsbronnen — betekent dat directe blootstelling.
...
- Anthropic Sonnet 4.6: van $3,00 naar $0,30 per miljoen [tokens](/begrippen#token) (90% korting).
...
<BBDisclosure title="Operationele modellen: kwaliteit naast beschikbaarheid monitoren">

Een [LLM](/begrippen#llm)-stack draait niet stabiel zonder bewaking, en bewaking met traditionele application-monitoring volstaat niet. Een technisch geslaagde aanvraag (server-status "OK") kan inhoudelijk een [hallucinatie](/begrippen#hallucinatie) bevatten; een latency-stijging (de wachttijd per aanvraag wordt langer) kan een retrieval-probleem maskeren (het ophalen van documenten uit de kennisbank werkt minder goed). Het bewaken en inzichtelijk maken van een LLM-stack vergt daarom een eigen kader.
...
- **Kosten**: [tokens](/begrippen#token) per aanroep (input én output), kosten per model. Doel: budget en anomaliedetectie.
...
- **[Drift](/begrippen#drift)**: 7-daagse trend in antwoordlengte; embedding-verdrift (de wiskundige representaties van nieuwe content schuiven weg van die van oude content). Doel: stille degradatie zichtbaar maken.
...
- **LLM03 Toeleveringsketen**: gemanipuleerde pre-trained modellen, vergiftigde datasets, kwetsbare bibliotheken, gecompromitteerde [MCP](/begrippen#mcp)-tools. Open-source modellen van Hugging Face kunnen verborgen kwaadaardige code bevatten die wordt uitgevoerd zodra je het model in een Python-omgeving inlaadt (in jargon: *malicious pickling*). Mitigaties: cryptografische checksums (wiskundige fingerprints zoals SHA-256 waarmee je controleert of een bestand onveranderd is), formele acceptatietesten van de leverancier voordat het model in productie gaat, geautomatiseerde scans van het modelbestand op verdachte code-patronen (bijvoorbeeld met de tool ModelAudit), en gedetailleerde logging van trainingswijzigingen.
...
- **[Knowledge](/framework/knowledge) (BB_01: domein-expertise externaliseren tot reproduceerbare kwaliteitscriteria)**: welk type model past bij de kennisstructuur? Vaak een combinatie: een [embedding-model](/begrippen#embedding-model) voor retrieval, een [LLM](/begrippen#llm) voor generatie, een classifier voor categorisering.
- **[Client Blueprint](/framework/client-blueprint) (BB_02: van klantvraag naar gestructureerd AI-ontwerp)**: de blueprint bepaalt de taken; Model Engines bepalen waarmee die taken worden uitgevoerd. Zonder evaluatie-set in de blueprint kun je geen verantwoorde modelkeuze maken.
...
- **[Tool Integration](/framework/tool-integration) (BB_05: externe tools, [MCP](/begrippen#mcp)-servers en agent-tooling)**: tool-zware [agents](/begrippen#agent) stellen eisen aan het model (Claude Opus 4.7 leidt op MCP-Atlas met 77,3%); modelkeuze en tool-architectuur zijn gekoppeld.
