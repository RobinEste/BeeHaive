...
| 2026-04-27 | BB_06 Model Engines | Volledig — 7 BBDisclosures + slot-disclosure, modeltype-keuze als ontwerpvraag (LLM/klassiek-ML/regels-matrix), modeltype-landschap (LLMs/redeneer/multimodaal/wereld-modellen V-JEPA 2 + Genie 3 + NVIDIA Cosmos/diffusiemodellen/embeddings/klassieke ML), frontier-landschap april 2026 (Opus 4.7 vs GPT-5.4 vs Gemini 3.1 Pro met taak-specifieke benchmarks + Tokenizer-inflatie + BrowseComp-regressie), routing (Anthropic 5 patronen + Dekoninck cascade routing ETH Zürich), EU-soevereiniteit (CLOUD Act + Mistral + Mistral Forge + AWS European Sovereign Cloud + Ollama/vLLM/TGI/llama.cpp), kostenoptimalisatie (5 hefbomen + kwantisatie + distillatie), runtime/observability (5 pijlers + 4 incident-correlaties + OWASP LLM03/LLM04 + AI-BOM). 12 losse em-dashes en 2 bold-em-dashes uit eerste concept gerepareerd via /bb-write skill; "uptime" → "beschikbaarheid", "post-launch" → "na ingebruikname", "downstream" → "achterliggend", vision-language-kop → "Multimodale modellen voor tekst, beeld en video", on-premise-kop → "Lokaal of op eigen infrastructuur draaien", right-sizing-kop → "modelschaal afstemmen op de taak". Astro check 0 errors. |
| 2026-04-27 | BB_06 her-pass (convergentie Gemini deep-research + 14 clippings) | Versie-correcties via web-verificatie: GPT-5.4 → GPT-5.5 (release 23 apr 2026), DeepSeek V3.2 → V4 Flash + V4 Pro $1,74/$3,48 (preview 24 apr 2026, MIT-licentie, Huawei Ascend-chips), Gemini 3.1 Pro context 2M → 1M tokens, Mistral Large 3 ~$1-$2 → $0,50/$1,50, GPT-5.5 Pro $30/$180 toegevoegd. Inhoudelijke uitbreidingen: 5e beslis-as determinisme-vs-generatie, Wayve GAIA-2/LINGO-2 in wereld-modellen, architectonische tegenstelling Anthropic/OpenAI/Google (39-70% performance-loss bij over-engineering), 6e hefboom speculative decoding (3-10× latency-winst), Nationale AI Fabriek SURF/TNO Groningen (€200M EuroHPC), EU AI Act handhavingsstart 2 aug 2026 + €35M/7%-jaaromzet boetes + 10²⁵ FLOPs systemic-risk-drempel, Datadog stat HTTP 429 = 60% van LLM-fouten feb 2026, AI-BOM standaarden CycloneDX-AI + SPDX-AI 3.0.1. Post-edit pass: 2 nieuwe losse em-dashes opgeruimd. Astro check 0 errors. |
